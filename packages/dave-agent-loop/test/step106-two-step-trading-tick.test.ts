  ...
  console.log("\n[4] Limit/stop -> market conversion: all 4 pending types, both the 'already passed' (converts) and 'not yet passed' (unconverted, regression) cases...\n");
  {
    ...
    for (const c of cases) {
      ...
      // Not yet passed -> placed exactly as decided, unconverted (regression, the normal common case).
      {
        ...
        const outcome = await runAutonomousTick({ userId: OWNER, db, executor, provider });
        // A waiting LIMIT opens its pullback scalp only when asked for (one position aimed at the limit,
        // run as the $20 cycle); without it, or on a STOP order, just the order.
        const isLimit = c.action === "SELL_LIMIT";
        assert.equal(placedOrders.length, isLimit ? 2 : 1, `${c.action} (not passed): the pending order must still fire${isLimit ? ", with its pullback scalp" : ""}`);
        const placed = placedOrders[0];
        if (isLimit) {
          const scalpSide = c.action === "SELL_LIMIT" ? "buy" : "sell";
          const [a] = placedOrders.slice(1);
          assert.equal(a.type, scalpSide, `${c.action}: the scalp goes the other way (${scalpSide})`);
          assert.equal(a.tp, c.entryNotPassed, `${c.action}: the scalp aims exactly at the limit price`);
