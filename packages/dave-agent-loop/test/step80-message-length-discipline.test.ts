...
assert.ok(prompt.includes("Default to short, punchy replies"), "the real short-by-default instruction must be present");
assert.ok(prompt.includes("Only write long when the content genuinely needs it"), "the real 'only go long when needed' instruction must be present");
...
assert.ok(prompt.includes("never one dense wall of text"), "the real paragraph-break instruction must still be present");
