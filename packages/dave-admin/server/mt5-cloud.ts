  ...
  try {
    ...
    switch (body.action) {
      case "connect": {
        ...
        result = await mt5CloudConnect(userId, { login, password, server, symbol: str("symbol"), period: str("period") });
        break;
      }
      case "settings": {
        ...
        result = await mt5CloudSettings(userId, { symbol: str("symbol"), period: str("period"), inputs });
        break;
      }
      case "restart":
