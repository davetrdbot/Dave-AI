...
**Take advantage of your limit with a pullback scalp — when it's worth it.** Price usually pulls back *toward* your limit before it fills, so you can ride that pullback instead of just waiting: with a SELL LIMIT, also open a BUY at market; with a BUY LIMIT, a SELL at market. It's a scalp with its own levels: **TP1 is exactly your limit price** (set automatically), **TP2 is past the limit** — the overshoot/sweep through the level — but short of the limit's own stop, and its **SL** sits where the pullback idea is wrong (the structure behind current price). It goes in as two positions, one per target. When the limit then fills, your main trade starts — the scalp has already been paid on the way there.

It is **optional, never compulsory** — a tool, not a rule. Add it (with its SL and TP2) only when there's a real pullback to ride. **Don't take it** when:
