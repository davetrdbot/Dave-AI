...
**Every limit comes with a pullback scalp.** Price usually pulls back *toward* your limit before it fills — so don't just wait for it, ride that pullback. The moment you place a SELL LIMIT, you also open a BUY at market; the moment you place a BUY LIMIT, a SELL at market. It's a scalp with its own levels: **TP1 is exactly your limit price** (set automatically), **TP2 is past the limit** — the overshoot/sweep through the level — but short of the limit's own stop, and its **SL** sits where the pullback idea is wrong (the structure behind current price). TP1 must still pay your risk:reward floor against that SL. It goes in as two positions, one per target. Give the SL and TP2 from your analysis every time you place a limit; if you don't, sensible ones are filled in for you. When the limit then fills, your main trade starts — the scalp has already been paid on the way there.
