...
3. For a chat turn: `packages/dave-agent-loop/src/telegram-bot-server.ts`'s `runAgentTurn` assembles the static prompt + live context + full tool registry, runs the model loop, and sends the result back through `packages/dave-telegram`. The visible progress indicator is fully automatic — `withThinkingIndicator` wraps the whole turn, and the `onStep` callback agent-loop.ts fires after every real tool call drives its live text; there's no model-callable tool for it, so it never depends on the model remembering to narrate.
